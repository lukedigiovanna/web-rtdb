...
CommandParseError::CommandParseError(std::string msg) : std::runtime_error(msg) {}
...
void advanceString(scit& it, const scit& end, std::stringstream& ss) {
    ...
    while (*it != '\"') {
        if (it == end) {
            throw CommandParseError("Error parsing string: missing end quotation in " + ss.str());
        }
        char c = *it;
        if (c == '\\') {
            if (it + 1 == end) {
            ...
            if (f == escChars.end()) {
                throw CommandParseError("Error parsing string: invalid escape sequence \\" + std::string{nc});
            }
            ss << f->second;
            it++;
        }
...
CommandTokenVector tokenize(std::string commandString) {
    ...
    while (it != end) {
        ...
        if (c == '\"') {
            ...
            if (std::find(
                commandOperationKws.begin(), commandOperationKws.end(), ss.str()) 
            ...
            throw CommandParseError("Unexpected character while tokenizing: " + std::string{c});
        }
