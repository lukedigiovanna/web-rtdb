...
ValueParseError::ValueParseError(std::string msg)
...
Value::Value(const std::string& stringVal) : d_type(e_STRING), d_value(stringVal) {

}

Value::Value(const Value::JsonObject& jsonObjVal) : d_type(e_JSON_OBJECT), d_value(jsonObjVal) {

}

Value::Value(const Value::JsonArray& jsonArrayVal) : d_type(e_JSON_ARRAY), d_value(jsonArrayVal) {
...
Value Value::parse(scit& it, const scit& strEnd) {
    if (*it == '\"') {
        ...
        throw ValueParseError("Unexpected token while parsing value: " + std::string(1, *it));
    }
}
