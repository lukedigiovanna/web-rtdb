    ...
    for (size_t i = 0; i < len; i++) {
        if (str[i] == '\"') {
            ...
            if (!inQuote) {
                ...
                if (i + 1 >= len || str[i + 1] == ' ') {
                    args.push_back(std::string(str + lastTokenIndex + 1,
                                               i - lastTokenIndex));
                    lastTokenIndex = i + 2;
                    i++;
                } else {
                    return -1; // Cannot have a non-space character after a
                               // terminating quote
                }
            }
        } else if (!inQuote && str[i] == ' ') {
            args.push_back(
                std::string(str + lastTokenIndex, i - lastTokenIndex));
            lastTokenIndex = i + 1;
        }
    }
...
int main(int argc, char *argv[]) {
    ...
    std::pair<std::string, std::function<int(std::vector<std::string> &, int,
                                             char *, size_t &)>>
        functions[NUM_FUNCTIONS] = {
            {"PING",
             [](std::vector<std::string> &args, int sockfd, char *buffer,
                size_t &length) -> int {
                 strcpy(buffer, "PONG");
                 length = 5;
                 return 0;
            ...
            {"SET",
             [&store, &subscribers](std::vector<std::string> &args, int sockfd,
                                    char *buffer, size_t &length) -> int {
                 if (args.size() != 3) {
                     return -1;
                 }
                 ...
                 for (int subscriber : subscribers[key]) {
                     // std::cout << "Sending: " << buffer << " to subscriber: "
                     // << subscriber << std::endl;
            ...
            {"QUERY",
             [&store](std::vector<std::string> &args, int sockfd, char *buffer,
                      size_t &length) -> int {
                 if (args.size() != 2) {
                     return -1;
                 }
    ...
    if (bind(sockfd, (const sockaddr *)&serverAddress, sizeof(serverAddress)) <
        0) {
        throw std::runtime_error("Error binding socket");
    }

    // extract the IP address
    std::cout << "Listening on: " << formatIPAddress(serverAddress)
              << std::endl;
    ...
    while (true) {
        ...
        for (size_t i = 0; i < fds.size(); i++) {
            if (fds[i].revents & POLLIN) { // read event
                if (fds[i].fd ==
                    sockfd) { // on server socket (i.e. new connection)
                    sockaddr_in clientAddress;
                    socklen_t addrlen = sizeof(clientAddress);
                    int clientfd =
                        accept(sockfd, (sockaddr *)&clientAddress, &addrlen);
                    if (clientfd >= 0) {
                    ...
                    if (n > 0) {
                        ...
                        if (splitArgs(msg, args) < 0) {
                            std::cerr << "Error parsing message: " << msg
                                      << std::endl;
                            send(fds[i].fd, "-ERROR error parsing command", 29,
                                 0);
                            continue;
                        }

                        if (args.size() == 0) {
                            std::cerr << "Error: Got empty message"
                                      << std::endl;
                        ...
                        for (size_t j = 0; j < NUM_FUNCTIONS; j++) {
                            if (functions[j].first == args[0]) {
                                ...
                                size_t len;
                                if (functions[j].second(args, fds[i].fd, buffer,
                                                        len) < 0) {
                                    std::cerr << "Error occurred" << std::endl;
                                    send(fds[i].fd,
                                         "-ERROR unknown error occurred", 30,
                                         0);
                                } else {
                        ...
                        std::string data =
                            "-ERROR unrecognized command: " + std::string(msg);
